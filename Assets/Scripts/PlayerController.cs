...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (!inputOverride)
        {
            if (Physics.Raycast(pCamera.transform.position, pCamera.transform.forward, out hit, 3))
            {
                if (hit.collider.CompareTag("RayNPCTrigger"))
                {
                    if (interactPressed)
                    {
                        interactPressed = false;
    ...
    public void SetInputOverride(bool inputOverride)
    {
        this.inputOverride = inputOverride;
    }

    public bool GetInputOverride()
    {
        return inputOverride;
    }

    public void SetInteractPressed(bool interactPressed)
    {
